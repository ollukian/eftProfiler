...
int main(int argc, char* argv[]) {
    ...
    eft::Application::Init(argc, argv);
    eft::Application::Run();

    eft::Application::Finalize();
    if (eft::Application::isErrored()) {
        EFT_PROF_CRITICAL("Application finished with errors");
        return 1;
    }
    EFT_PROF_INFO("Application finished successfully");
    return 0;
    //const string request = commandLineArgs->GetKey("task");
