...
submit_one_worker()
{
  ...
  echo "####################################################################################################################" >> "${current_script}"
  echo "# script automatically created by run_remote version @ 16Nov22 / Lukianchuk Aleksei lukianchuk.aleksei@gmail.com  ##" >> "${current_script}"
  echo "####################################################################################################################" >> "${current_script}"
  ...
  echo "sh job_script.sh --task compute_ranking --errors ${EFT_ERRORS_HANDLING} --poi_init_val 0 --worker_id ${worker_id} --poi ${POI} --res_path ${EFT_SAVE_RES_TO} --no_gamma --ws_path ${WS_PATH}" >> "${current_script}"
