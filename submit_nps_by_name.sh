...
submit_one_worker()
{
  ...
  echo "####################################################################################################################" >> "${current_script}"
  echo "# script automatically created by run_remote version @ 14Dec22 / Lukianchuk Aleksei lukianchuk.aleksei@gmail.com  ##" >> "${current_script}"
  echo "# eftProfiler version 1.0.7 @ 14Dec22                                                                             ##" >> "${current_script}"
  echo "####################################################################################################################" >> "${current_script}"
  ...
  echo "sh job_script.sh --task compute_ranking --np_name ${NP_NAME} --errors ${EFT_ERRORS_HANDLING} --poi_init_val ${POI_INIT_VAL} --worker_id ${worker_id} --poi ${POI} --res_path ${EFT_SAVE_RES_TO} --no_gamma --ws_path ${WS_PATH} --comb_pdf ${PDF_NAME} --strategy ${STRATEGY} ${OTHER_OPTIONS}" >> "${current_script}"
